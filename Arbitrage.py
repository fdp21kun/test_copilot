...
#    'Bank': '1615.T',  # 銀行行ETF
#    'Build': '1619.T',  # 建設業ETF
#    'REIT': '1345.T',  # REIT ETF
#    sym1: '1621.T',  # 医薬品ETF
#    'Enagy': '1618.T',  # エネルギーETF
    'N225CME': 'NIY=F',  # N225CME先物
}
...
# sym2 を sym1 に重ねるための動的スケーリング係数の計算（平均値の比率を使用）
scaling_factor1 = merged_data[sym1].mean() / merged_data[sym2].mean()
merged_data['Scaled#1'] = merged_data[sym2] * scaling_factor1
scaling_factor2 = merged_data[sym2].mean() / merged_data[sym1].mean()
merged_data['Scaled#2'] = merged_data[sym1] * scaling_factor2
...
# 日経ETFの差を計算
merged_data['diff_data'] = merged_data['Scaled#1'] - merged_data[sym1]
# merged_data['diff_data'] = merged_data[sym2] - merged_data[sym1]
...
for column in merged_data.columns:
    if column == 'diff_data': continue
    plt.plot(merged_data.index, merged_data[column], label=column)

plt.title("Nikkei 225, Dow Jones, DowETF, and USD/JPY x X", fontsize=16)
